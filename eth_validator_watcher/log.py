...
def log_multiple_entries(
    ...
    msg_validators_shell = (
        f'{", ".join([shorten_validator(v) for v in validators])} and more'
    )
    msg_shell = f"{color}{emoji} Validator(s) {msg_validators_shell}{label_msg_shell} {msg}{COLOR_RESET}"
    logging.info(msg_shell)

    msg_validators_slack = (
        f'{", ".join([beaconcha_validator_link(cfg, v) for v in validators])} and more'
    )
...
def log_details(
    cfg: Config,
    registry: WatchedValidators,
    metrics: dict[str, MetricsByLabel],
    current_slot: int,
):
    ...
    for slot, validator in m.details_proposed_blocks:
        log_single_entry(
            cfg, validator, registry, f"proposed a block", "🏅", slot, COLOR_BOLD_GREEN
        )

    for slot, validator in m.details_missed_blocks:
        log_single_entry(
            cfg, validator, registry, f"likely missed a block", "😩", slot, COLOR_RED
        )
    ...
    if m.details_missed_attestations:
        ...
        if current_slot % 32 == SLOT_FOR_MISSED_ATTESTATIONS_PROCESS:
            log_multiple_entries(
                cfg,
                m.details_missed_attestations,
                registry,
                f"missed an attestation",
                "😞",
                COLOR_YELLOW,
            )
