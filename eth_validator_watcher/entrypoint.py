...
from .rewards import (
    process_block_reward,
    process_rewards,
    process_sync_committee_rewards,
)
from .utils import (
...
class ValidatorWatcher:
    ...
    def _reload_config(self) -> None:
        ...
        try:
            if (
                not self._cfg
                or self._cfg_path.stat().st_mtime != self._cfg_last_modified
            ):
        ...
        if (
            self._beacon is None
            or self._beacon.get_url() != self._cfg.beacon_url
            or self._beacon.get_timeout_sec() != self._cfg.beacon_timeout_sec
        ):
            self._beacon = Beacon(self._cfg.beacon_url, self._cfg.beacon_timeout_sec)

    def _update_metrics(
        self, watched_validators: WatchedValidators, epoch: int, slot: int
    ) -> None:
        ...
        self._metrics.eth_current_price_dollars.labels(network).set(
        ...
        metrics = compute_validator_metrics(watched_validators.get_validators())
        ...
        for label, m in metrics.items():
            for status in Validators.DataItem.StatusEnum:
                ...
                self._metrics.eth_validator_status_count.labels(
                    label, status, network
                ).set(value)

        for label, m in metrics.items():
            self._metrics.eth_suboptimal_sources_rate.labels(label, network).set(
                pct(m.suboptimal_source_count, m.optimal_source_count)
            )
            self._metrics.eth_suboptimal_targets_rate.labels(label, network).set(
                pct(m.suboptimal_target_count, m.optimal_target_count)
            )
            self._metrics.eth_suboptimal_heads_rate.labels(label, network).set(
                pct(m.suboptimal_head_count, m.optimal_head_count)
            )

            self._metrics.eth_ideal_consensus_rewards_gwei.labels(label, network).set(
                m.ideal_consensus_reward
            )
            self._metrics.eth_actual_consensus_rewards_gwei.labels(label, network).set(
                m.actual_consensus_reward
            )
            self._metrics.eth_block_rewards_gwei.labels(label, network).set(
                m.block_reward
            )
            ...
            self._metrics.eth_missed_attestations_count.labels(label, network).set(
                m.missed_attestations
            )
            self._metrics.eth_missed_consecutive_attestations_count.labels(
                label, network
            ).set(m.missed_consecutive_attestations)
            self._metrics.eth_slashed_validators_count.labels(label, network).set(
                m.validator_slashes
            )
            ...
            self._metrics.eth_block_proposals_head_total.labels(label, network).inc(
                m.proposed_blocks
            )
            self._metrics.eth_missed_block_proposals_head_total.labels(
                label, network
            ).inc(m.missed_blocks)
            self._metrics.eth_block_proposals_finalized_total.labels(
                label, network
            ).inc(m.proposed_blocks_finalized)
            self._metrics.eth_missed_block_proposals_finalized_total.labels(
                label, network
            ).inc(m.missed_blocks_finalized)

            self._metrics.eth_future_block_proposals.labels(label, network).set(
                m.future_blocks_proposal
            )
    ...
    def run(self) -> None:
        ...
        slack_send(
            self._cfg,
            f"🚀 *Ethereum Validator Watcher* started on {self._cfg.network}, watching {len(self._cfg.watched_keys)} validators",
        )

        while True:
            logging.info(f"🔨 Processing slot {slot}")

            last_finalized_slot = self._beacon.get_header(
                BlockIdentierType.FINALIZED
            ).data.header.message.slot
            self._schedule.update(
                self._beacon, slot, last_processed_finalized_slot, last_finalized_slot
            )

            if beacon_validators == None or (
                slot % self._spec.data.SLOTS_PER_EPOCH == 0
            ):
            ...
            if validators_liveness == None or (
                slot % self._spec.data.SLOTS_PER_EPOCH
                == SLOT_FOR_MISSED_ATTESTATIONS_PROCESS
            ):
                logging.info("🔨 Processing validator liveness")
                validators_liveness = self._beacon.get_validators_liveness(
                    epoch - 1, watched_validators.get_indexes()
                )
            ...
            if rewards == None or (
                slot % self._spec.data.SLOTS_PER_EPOCH == SLOT_FOR_REWARDS_PROCESS
            ):
            ...
            logging.info("🔨 Processing block reward")
            block_reward = self._beacon.get_block_reward(slot)
            process_block_reward(watched_validators, block_reward)
            ...
            while (
                ...
                and last_processed_finalized_slot < last_finalized_slot
            ):
                logging.info(
                    f"🔨 Processing finalized slot from {last_processed_finalized_slot or last_finalized_slot} to {last_finalized_slot}"
                )
            ...
            if (
                self._cfg.replay_end_at_ts
                and self._clock.now() >= self._cfg.replay_end_at_ts
            ):
